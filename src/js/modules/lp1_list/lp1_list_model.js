...
console.log('lp1_list.model start');
...
lp1_list.model = {
	build_list_html:function(data){
        ...
        var stat_carcas = '<div class="like"><div class="help tooltiped" title="Считаются только переходы по РК - посещения страницы с UTM метками в ссылке">?</div><p>' + view + '</p><span>переходов</span></div><div class="like"><p>' + lead + '</p><span>заявок</span></div><div class="like"><p>' + conv + '</p><span>конверсия</span></div>';
               ...
               if(data.length - i > parseInt(global_user_data.limits['1Page'].limit)){

               		over_limit = ' over'

               }
		    //создание строки киркаса
		    carcas += '<div class="project'+over_limit+'" data-id="' + data[i].id + '"><div class="block">' + name + '</div><div class="block">' + domiain_url + '</div><div class="like_gr">'+stat_carcas+'</div><div class="block">' + delete_link+'<a class="edit page_edit" href="/lp1/edit?i=' + data[i].id + '">Редактировать</a>' + yar_link + '</div><div class="block">' + server_url + '</div></div>';
