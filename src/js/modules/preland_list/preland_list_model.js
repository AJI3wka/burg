...
console.log('preland_list.model start');
	...
	build_list_html:function(data){
		...
		for (var i = data.length - 1; i >= 0; i--) {
	        ...
	        var stat_carcas = '<div class="like"><p>' + view + '</p><span>Посещений</span></div><div class="like"><p>' + lead + '</p><span>заказов</span></div><div class="like"><p>' + conv + '</p><span>конверсия</span></div>';
               ...
               if(data.length - i > parseInt(global_user_data.limits['Preland'].limit)){
		    ...
		    carcas += '<div class="project'+over_limit+'" data-id="' + data[i].id + '"><div class="block">' + name + '</div><div class="block">' + domiain_url + '</div><div class="like_gr">'+stat_carcas+'</div><div class="block">'+ delete_link +'<a class="edit page_edit" href="/preland/edit?i=' + data[i].id + '">Редактировать</a>' + yar_link + '</div><div class="block">' + server_url + '</div></div>';

		}
	}
		carcas+='<div class="project last">';
		carcas+='<div class="new"><img src="'+source_url+'/img/plus.png" alt height="66" width="67"><p>Cоздать преленд самостоятельно</p></div>';
