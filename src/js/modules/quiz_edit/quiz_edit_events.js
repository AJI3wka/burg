...
console.log('quiz_edit.events start');
...
quiz_edit.events = {
    ...
    unbind_input_listening: function() {
        ...
        quiz_edit.view.wrap.find(quiz_edit.model.get_input_selectors()).unbind('blur', quiz_edit.events.input_update_on_blur); //на все эти селекторы удаляем с евента выхода с фокуса автосоранения


    },
    bind_input_listening: function() {

        quiz_edit.view.wrap.find(quiz_edit.model.get_input_selectors()).bind('blur', quiz_edit.events.input_update_on_blur); //на все эти селекторы ставим на выход с фокуса евент автосоранения
    ...
    input_update_on_blur: function(event) {
        ////alert('blur');
        //
        if (quiz_edit.events.update_on_blur_timer) {
            clearTimeout(quiz_edit.events.update_on_blur_timer);
        }
        ...
        // quiz_edit.events.update_on_blur_timer = setTimeout(function(){
        ...
        if (!$this.hasClass('error-input')) { //если нет ошибок
            ...
            if ($this.attr('data-last-pushed') != $this.val() && $this.attr('data-last-pushed') != $this.val().replace(/\n/g,'<br>') ) { //если значение и последнее сохраненное значение которое хранится в атрибуте отличаются




                quiz_edit.model.update_cur_from_elem($this);


                //quiz_edit.events.update_on_blur_timer = setTimeout(function() {

                    quiz_edit.view.start_autosave($this);

                    quiz_edit.controller.save(true, function() {

                        quiz_edit.view.end_autosave($this);
    ...
    trig_click_handler: function() {
        ...
        if (!$this.hasClass('active')) {
            ...
            if (target == 'bg_video') {
                ...
                // $wrap.find('.bg_img_m').val('0').trigger('blur');
                // $wrap.find('.bg_img_d').val('0').trigger('blur');
            } else
            if (target == 'content_autoplay') {
                $wrap.find('.content_autoplay').val('1').trigger('blur');
            } else if(target == 'color_theme') {
                $wrap.find('.color_theme').val('light').trigger('blur');
            } else
            ...
            if (target == 'bg_video') {
                ...
                // $wrap.find('.bg_img_m').val('').trigger('blur');
                // $wrap.find('.bg_img_d').val('').trigger('blur');
            } else
            if (target == 'content_autoplay') {
                $wrap.find('.content_autoplay').val('0').trigger('blur');
            } else if(target == 'color_theme') {
                $wrap.find('.color_theme').val('dark').trigger('blur');
            } else
            if (target == 'content_video') {
    ...
    logo_change_handler: function(e) {
        ...
        var $wrap_in = quiz_edit.view.wrap;
        var $label = $(this).closest('.hidden-box').parent().find('label');

        quiz_edit.controller.upload_img(this, 'logo', function(data) {
    ...
    bg_change_handler: function(e) {
        ...
        var $label = $(this).closest('.hidden-box').parent().find('label');
        quiz_edit.controller.upload_img(this, 'bg', function(data) {
            var d = new Date();
    ...
    file_input_change_handler: function(e) {
        ...
        var $label = $(this).closest('.hidden-box').parent().find('label');
        quiz_edit.controller.upload_img(this, type, function(data) {
    ...
    //     if (!quiz_edit.view.wrap.is(':visible')) {
    //         return false;
    //     }
    //     quiz_edit.view.live_change_frame($(this));
    // },
    // resize_helper: function() {
    //     if (quiz_edit.events.resize_window_timer) {
    //         clearTimeout(quiz_edit.events.resize_window_timer);
    //     }
    //     quiz_edit.events.resize_window_timer = setTimeout(function() {

    //         if (quiz_edit.view.wrap.is(':visible') && $(window).width() > 767) {
    ...
    valid_enter_tab_keypress: function(e) {
        var can_enter = false;
        if ($(this).is('[data-input-type="q_pre_form_offer"]')){
            can_enter = true;
        }
        if (e.which == 13 && !can_enter || e.keyCode == 9) {
            e.preventDefault();
            e.stopPropagation();
            ...
            console.log('$page.scrollTop() = ',$page.parent().parent().scrollTop());
    ...
    rebind: function() {
        ...
        // quiz_edit.view.wrap = $('#body');

        // quiz_edit.events.unbind_input_listening();
        // quiz_edit.events.bind_input_listening();

quiz_edit.view.wrap.find(quiz_edit.model.get_input_selectors()).unbind('focus', quiz_edit.events.pusl_inst.focus_h);
quiz_edit.view.wrap.find(quiz_edit.model.get_input_selectors()).bind('focus', quiz_edit.events.pusl_inst.focus_h); //на все эти селекторы ставим на выход с фокуса евент автосоранения

quiz_edit.view.wrap.find(quiz_edit.model.get_input_selectors()).unbind('blur', quiz_edit.events.pusl_inst.blur_h);
quiz_edit.view.wrap.find(quiz_edit.model.get_input_selectors()).bind('blur', quiz_edit.events.pusl_inst.blur_h); //на все эти селекторы ставим на выход с фокуса евент автосоранения
        ...
        $wrap.find('input.valid,textarea.valid').unbind('keydown', quiz_edit.events.valid_enter_tab_keypress);
        $wrap.find('input.valid,textarea.valid').bind('keydown', quiz_edit.events.valid_enter_tab_keypress);


        $wrap.find('#upfile1').unbind('change', quiz_edit.events.logo_change_handler);
        $wrap.find('#upfile1').bind('change', quiz_edit.events.logo_change_handler);

        $wrap.find('#upfile2').unbind('change', quiz_edit.events.bg_change_handler);
        $wrap.find('#upfile2').bind('change', quiz_edit.events.bg_change_handler);

        $wrap.find('.new_uploads').unbind('change', quiz_edit.events.file_input_change_handler);
        $wrap.find('.new_uploads').bind('change', quiz_edit.events.file_input_change_handler);


        $wrap.find('.trig_btn').unbind('click', quiz_edit.events.trig_click_handler);
        $wrap.find('.trig_btn').bind('click', quiz_edit.events.trig_click_handler);

        $wrap.find('.trig_btn[data-target="form_input"]').unbind('click', quiz_edit.events.form_check_click_hendler);
        $wrap.find('.trig_btn[data-target="form_input"]').bind('click', quiz_edit.events.form_check_click_hendler);

        $wrap.find('input[name="bg_shadow"]').unbind('input', quiz_edit.events.bg_shadow_input_handler);
        $wrap.find('input[name="bg_shadow"]').bind('input', quiz_edit.events.bg_shadow_input_handler);

        $wrap.find('input.youtube').unbind('blur input change', quiz_edit.events.youtube_helper);
        $wrap.find('input.youtube').bind('blur input change', quiz_edit.events.youtube_helper);
        ...
        $wrap.find('.publish_btn,.fast_publish').click(function(e) {
            ...
            validate.wrap($wrap, function() {
                ...
                quiz_edit.model.send_full_data(function() {

                    var current = +$wrap.find('.steps_pager_item.active').attr('data-step');
                    var next =  +$wrap.find('.steps_pager_item').last().attr('data-step');
                    quiz_edit.view.go_to_step(current, next);

                });

            });
                if ($wrap.find('.error-input,.error-btn').length>0) {
                    var $first = $wrap.find('.error-input,.error-btn').first();

                    var next = $first.closest('.step').attr('data-step')
        ...
        $wrap.find('.text_check_btn').click(function(e) {
            ...
            if (window.glvrd) {
                ...
                quiz_edit.view.glvrd.wrap.addClass('opened');
            }
        });
        ...
        $wrap.find('.trig[data-target="content_video"]').unbind('click', quiz_edit.events.content_video_helper);
        $wrap.find('.trig[data-target="content_video"]').bind('click', quiz_edit.events.content_video_helper);

        // $wrap.find('.pre_form_offer').unbind('change input blur', quiz_edit.events.pre_offer_helper);
        // $wrap.find('.pre_form_offer').bind('change input blur', quiz_edit.events.pre_offer_helper);
        ...
        // $wrap.find('.change_iframe').unbind('blur input change', quiz_edit.events.change_iframe_helper);
        // $wrap.find('.change_iframe').bind('blur input change', quiz_edit.events.change_iframe_helper);

        // $(window).unbind('resize', quiz_edit.events.resize_helper);
        // $(window).bind('resize', quiz_edit.events.resize_helper);
        ...
        // 	clearTimeout(quiz_edit.view.frame_to_fullscreen_timer);
        // 	clearTimeout(quiz_edit.view.frame_from_fullscreen_timer);
        // 	quiz_edit.view.frame_to_fullscreen_timer = setTimeout(function(){
        // 		if ($(window).width() > 767) {
        // 			$wrap.addClass('iframe_active');
        // 			quiz_edit.view.resize_edit_frame(true);
        // 		}
        // 	}, 1500);
        // });
        ...
        // 	clearTimeout(quiz_edit.view.frame_to_fullscreen_timer);
        // 	clearTimeout(quiz_edit.view.frame_from_fullscreen_timer);
        // 	quiz_edit.view.frame_from_fullscreen_timer = setTimeout(function(){
        // 		$wrap.addClass('iframe_active');
        // 		quiz_edit.view.resize_edit_frame(true,true);
        // 	}, 1500);
        // });
        ...
        $('#preview-site').unbind('click').click(function(e){
            ...
            if(if_defined(href)){
                ...
                $('#preview-site-frame').find('.iframe-wrap').append('<iframe src="'+href+'"></iframe>').show();
            }
        ...
        $(window).unbind('resize',quiz_edit.events.bx_resize_fix);
        $(window).on('resize',quiz_edit.events.bx_resize_fix);
        ...
        $wrap.find('.inner_domain').blur(function(){
            ...
            if(val != $(this).attr('data-last-save')){
                ...
                if(if_defined(val)){
                    ...
                    quiz_edit.controller.check_atom_domain(val,function(data){
                        console.log('data_check = ',data);
                        if(data.checkAtomDomain){
                            $this.closest('.field_wrap').find('p.error_info').html('');
                            quiz_edit.controller.attach_atom_domain(val,function(data){
    ...
    glvrd:{
        ...
        i_focus_h:function(e){
            quiz_edit.events.glvrd.make_focused(this,true);
        },
        i_blur_h:function(e){
            //e.preventDefault();
            //e.stopPropagation();

            if (quiz_edit.events.glvrd.i_key_timeout) {
                 clearTimeout(quiz_edit.events.glvrd.i_key_timeout)
            }
            quiz_edit.view.glvrd.wrap.find('#c_edit_f').removeClass('g_focused').removeAttr('id');
            var $elem = $(this);
            // setTimeout(function(){

                quiz_edit.model.glvrd.update_real_val($elem);   
            // },2000);         
        },
        i_key_time:3000,
        i_key_timeout:false,
        ...
        i_keydown_h:function(e){
            ...
            quiz_edit.events.glvrd.make_focused(this);

            if (quiz_edit.events.glvrd.i_key_timeout) {
                clearTimeout(quiz_edit.events.glvrd.i_key_timeout)
            }
            var $inp = $(this);
            quiz_edit.events.glvrd.i_key_timeout = setTimeout(function(){
                quiz_edit.model.glvrd.update_real_val($inp);

            },3000);

        },
        rebind_inputs:function(){
            var $inps = quiz_edit.view.glvrd.wrap.find('.g_textarea,.g_input');
            var $frg = $inps.find('.fragment');

            $frg.unbind('mouseenter mouseleave');
            $frg.hover(function() {
                quiz_edit.view.glvrd.show_desc($(this).attr('data-ind'));
            }, function() {
                quiz_edit.view.glvrd.hide_desc();
        ...
        rebind:function(){
            // quiz_edit.model.glvrd.full_data = {
            //     descriptor:'',
            //     offer_h2:'',
            //     offer_h1:'',
            //     pre_form_offer:''
            // };
            var $inps = quiz_edit.view.wrap.find('input.valid,textarea.valid');

            quiz_edit.view.glvrd.wrap.find('.close').unbind('click');
            quiz_edit.view.glvrd.wrap.find('.close').click(function(e){

                quiz_edit.view.glvrd.wrap.removeClass('opened');
