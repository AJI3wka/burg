...
console.log('lp1_edit.model start');
...
lp1_edit.model = {
	...
	get_input_selectors:function(){
	    ...
	    for (var i = lp1_edit.model.inputs.length - 1; i >= 0; i--) {//по всему вектору занчений
	...
	update_cur_from_elem:function($this){
        ...
        for (var i = lp1_edit.model.inputs.length - 1; i >= 0; i--) {//по всей длинне вектора полей

            var inp = lp1_edit.model.inputs[i];//элемент вектора
            ...
            if (inp.hasOwnProperty('key')) {//если у елемента вектора есть параметр key
                ...
                if ($this.is(inp.selector)) {//если этот элемент это селектор с элемента вектора
                    ...
                    if (inp.key.indexOf('content') > -1) {//если ключ в елементе вектора имеет content(это значение ввнутри content)
                        ...
                        if (lp1_edit.model.cur_data.content == null || lp1_edit.model.cur_data.content == 0) {//если в глобальной переменной нету контента создаем его
                            lp1_edit.model.cur_data.content = {};
                        }
 						...
 						if(content_key == 'act_email'){
                        	...
                        	lp1_edit.model.cur_data.content[content_key] = n_val;


                        }else{

                        	lp1_edit.model.cur_data.content[content_key] = cur_val;
                        ...
                        if (content_key == 'offer_h1' || content_key == 'form_head' || content_key == 'descriptor') {
                            ...
                            lp1_edit.model.cur_data.content[content_key] = cur_val.replace(/^\s+|\s+$/g, ' ')
                        }

                        if (content_key == 'page_email' && cur_val.length == 0) {
                            //если не указан page_email, то присваем ему значение act_email
                            lp1_edit.model.cur_data.content[content_key] = lp1_edit.model.cur_data.content.act_email
                        }

                        if (content_key == 'bg_video' && cur_val != '0') {
                            //если это bg_video используем ютуб парсер на значении(возвращает ИД виедо ютуб)
                            lp1_edit.model.cur_data.content[content_key] = youtube_parser(cur_val);
                        }
                        ...
                        lp1_edit.model.cur_data[inp.key] = cur_val;
		...
		lp1_edit.model.cur_data.content.gentime = new Date().getTime();

        /*-------- наполнение объекта lp1_edit.model.cur_data актуальным контентом, конец ---------*/
	},
	parse_full_data:function(){
	    ...
	    lp1_edit.model.cur_data.content = content;

	    lp1_edit.model.cur_data.template = 'wov';

	    lp1_edit.model.cur_data.name = land_name;

	    lp1_edit.model.cur_data.domain = domain;
	...
	send_full_data:function(callback){
	    ...
	    lp1_edit.model.parse_full_data();
	...
	init_edit_by_id:function(id){
		...
		//lp1_edit.view.insert_edit_frame(function(){
	        ...
	        lp1_edit.controller.get(id,function(data){
	            

		        lp1_edit.view.paste_values(data.data);//вставить полученные данные в редактор указанного типа
	...
	init_helpers:function(){
		var $wrap = lp1_edit.view.wrap.find('.helpers');
	...
	load_quiz_list:function(){
	    ...
	    $("#quiz_panel").load(source_url+"/html/parts/quiz/panel.html", function() {
		  //lp1_edit.view.panel = $("#quiz_panel");
	...
	remove_quiz_from_page:function(){
		...
		if (if_defined(quiz_editor)) {
			quiz_editor.controller.attach_quiz_to_page(null,lp1_edit.model.cur_data.id);
		}

	},
	load_quiz:function(data){

		// if(if_defined(lp1_edit.view.edit_frame) && if_defined(lp1_edit.view.edit_frame.atom_quiz)){
		// 	lp1_edit.view.edit_frame.atom_quiz.qiuz_data = Object.assign({},data);
		// 	//lp1_edit.view.edit_frame.atom_quiz.quiz_data = data;
		// 	lp1_edit.view.edit_frame.atom_quiz.load();
		// }
	...
	glvrd:{
		inited:false,
		init:function(){
			lp1_edit.view.glvrd.wrap = $('#glvrd_body');
			this.full_data = {
				descriptor:{
					text:'',
					num:0,
			...
			lp1_edit.view.glvrd.wrap.find('.g_i-wrap').html('');
			if (!if_defined(window.glvrd)) {
				...
				$.getScript("https://api.glvrd.ru/v1/glvrd.js", function(data, textStatus, jqxhr) {
					...
					lp1_edit.events.glvrd.rebind();

					lp1_edit.model.glvrd.inited = true;
        ...
        clean_test:function(){
            ...
            lp1_edit.view.glvrd.wrap.find('.g_i-wrap').html('');
        	lp1_edit.view.wrap.find('input.valid,textarea.valid').each(function(index, el) {
        ...
        test:function(){
        	...
        	var ta = lp1_edit.view.glvrd.wrap.find('.g_textarea');
            glvrd.proofread(test_string, function(result){
        		...
        		if (result.status="ok") {	
					...
					lp1_edit.view.wrap.find('#glvrd_all').html(score);
					lp1_edit.view.glvrd.wrap.find('.g_result').html(score).css('color',color);
					...
					lp1_edit.view.glvrd.wrap.find('.g_status').html(text);

        			var quiz_cuts = [];
	        		lp1_edit.model.glvrd.cur_res = result;
	        		...
	        		if (result.fragments.length>0) {
						...
						var $i_wrp = lp1_edit.view.glvrd.wrap.find('.g_i-wrap');
						for (var i = 0; i < inps.length; i++) {
							 for(var key in lp1_edit.model.glvrd.full_data){
							 	if (lp1_edit.model.glvrd.full_data.hasOwnProperty(key)) {

							 		if (lp1_edit.model.glvrd.full_data[key].num == i) {
							 			...
							 			if ($t_inp.length>0) {
							 				...
							 				//if (inps[i].length>5) {
								 				...
								 				$i_wrp.append('<p class="g_label">'+lp1_edit.model.glvrd.full_data[key].label+'</p>');
	        		...
	        		lp1_edit.events.glvrd.rebind_inputs();
        		}
        ...
        update_real_val:function($elem){
        	...
        	lp1_edit.view.wrap.find('input.valid[name="'+key+'"],textarea.valid[name="'+key+'"]').val(text).trigger('blur');
